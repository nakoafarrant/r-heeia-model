---
title: "He’eia Model Sensitivity Analysis"
...
fish_func6 <- function(t, state6, pars6) {
  with(as.list(c(state6, pars6)), {
    ...
    if(t < 20){
      W_No = (LU_No + No_inc*yr_curr)*Q_st
      W_Na = (LU_Na + Na_inc*yr_curr)*Q_st
      W_Ni = (LU_Ni + Ni_inc*yr_curr)*Q_st
    } else {
      W_No = (LU_No + No_inc*20)*Q_st
      W_Na = (LU_Na + Na_inc*20)*Q_st
      W_Ni = (LU_Ni + Ni_inc*20)*Q_st
    }
  ...
  # Algae density
  dalg <- (k_ag*(Na_fp+Ni_fp)/(k_SN+Na_fp+Ni_fp) -k_ra)*alg
  
  # Fish density
  
  # define catch rate
  if(step < 365*2)
    catch = 0 # No harvests during the first 2 years after stocking to allow the initial population to mature
  else
    catch = k_catch # after the first 2 years, harvest at the defined first order rate
  
   
  #dfish <- 1.08*fish - (M + catch)*fish # question the parentheses location from a units perspective
  ...
  return(list(c(No_st = dNo_st, Na_st = dNa_st, Ni_st = dNi_st, No_fp = dNo_fp, Na_fp = dNa_fp, Ni_fp = dNi_fp, No_sd = dNo_sd, Na_sd = dNa_sd, Ni_sd = dNi_sd, alg = dalg, fish = dfish, catch = dcatch, N = dN)))
...
pars_sens <- c(
      ...
      # based on doubling time of 1.4-4.4 years, r_growth = ln(2)/1.4 = 0.495 /yr to ln(2)/4.4 = 0.158 /yr, prior r (whatever that means) is 0.51
      M = M_mugil/365/4, # [1/6hr] rate of fish death from empirical Then formula, M_mugil = 0.346
      k_catch = 1.2/365/4*-1.2, # could be 0.343/yr based on Mexico study.
      # F_msy = 1.2 /year if harvest at 2 years using the 60cm Linf
      # file:///Users/Nakoa/Downloads/2016GallardoetalfisherydeMugelcephalusJMBO-5-151.pdf
      # fishbase also says you generally want Exploitation E to be about 0.5 which is F/Z = F/(F+M) so M and F should be approximately equal ideally for MSY
      # From Barman, Jana, Garg, Bhatnagar, https://link.springer.com/article/10.1007/s10499-004-2479-5
...
out_sens <- ode(y = state6, times = t, func = fish_func6, parms = pars_sens, method = "euler")
...
df_sens <- as.data.frame(out_sens)

fish_catch <- df_control %>% 
  dplyr::select(time, catch) %>% 
  dplyr::rename(Current = catch) %>% 
  mutate(Sensitivity = df_sens$catch) %>% 
  gather('Current', 'Sensitivity', key = "Scenario", value = "Mass")

fish_dens <- df_control %>% 
  dplyr::select(time, fish) %>% 
  dplyr::rename(Current = fish) %>% 
  mutate(Sensitivity = df_sens$fish) %>% 
  gather('Current', 'Sensitivity', key = "Scenario", value = "Density")
...
```{r}
sensitivity_pct = (tail(df_sens$catch, n=1)-tail(df_control$catch, n=1))/tail(df_control$catch, n=1)*100
