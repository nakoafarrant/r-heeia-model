...
# In our case, we are initializing this model to conditions on Jan. 1, 2014. The moon phase was a new moon (spring tide) and the tide was high in the early morning hours (flood conditions)
    ...
    # Specify temporal variability in He'eia stream flow rates
    # USGS Stream Gauge #16275000 He'eia Stream at Haiku Valley near Kaneohe, Oahu, HI
    # Data originally collected in cubic feet per second and converted to m^3 / day and then m^3 / 6 hours 
    ...
    # Below values are in units of m^3 / 6 hours
    # The first two years are specified using monthly averages from 2014 and 2015 because the nitrogen water quality data used to evaluate the model were collected between July 2014 and July 2015
    # After the first two years, monthly stream flow rates are specified using the monthly averages from 2013-2018
    if(t < 1) {
      if (step %% 366 < 32) {
          # Jan 2014
          Q_st = 3940/4
      }
      else if (step %% 366 > 31 && step %% 366 < 60){
          # Feb 2014
          Q_st = 6240/4
      }
      else if (step %% 366 > 59 && step %% 366 < 91) {
          # March 2014
          Q_st = 7660/4
      }
      else if (step %% 366 > 90 && step %% 366 < 121) {
          # April 2014
          Q_st = 4530/4
      }
      else if (step %% 366 > 120 && step %% 366 < 152) {
          # May 2014
          Q_st = 4990/4
      }
      else if (step %% 366 > 151 && step %% 366 < 182) {
          # June 2014
          Q_st = 4010/4
      }
      else if (step %% 366 > 181 && step %% 366 < 213) {
          # July 2014
          Q_st = 8860/4
      }
      else if (step %% 366 > 212 && step %% 366 < 244){
          # Aug 2014
          Q_st = 4620/4
      }
      else if (step %% 366 > 243 && step %% 366 < 274) {
          # Sept 2014
          Q_st = 4650/4
      }
      else if (step %% 366 > 273 && step %% 366 < 305) {
          # Oct 2014
          Q_st = 6560/4 
      }
      else if (step %% 366 > 98 && step %% 366 > 129) { 
          # Nov 2014
          Q_st = 4970/4 
      }
      else if (step %% 366 > 128 && step %% 366 < 160) {
         # Dec 2014 
         Q_st = 4580/4
      }
      else {
      ...
      if (step %% 366 < 32) {
       ...
       if (step %% 366 < 32) {
          # Jan six-year average (2013-2018)
          Q_st = 4650/4
      }
      else if (step %% 366 > 31 && step %% 366 < 60){
          # Feb six-year average (2013-2018)
          Q_st = 6610/4
      }
      else if (step %% 366 > 59 && step %% 366 < 91) {
          # March six-year average (2013-2018)
          Q_st = 6610/4
      }
      else if (step %% 366 > 90 && step %% 366 < 121) {
          # April six-year average (2013-2018)
          Q_st = 5630/4
      }
      else if (step %% 366 > 120 && step %% 366 < 152) {
          # May six-year average (2013-2018)
          Q_st = 6120/4
      }
      else if (step %% 366 > 151 && step %% 366 < 182) {
          # June six-year average (2013-2018)
          Q_st = 4650/4
      }
      else if (step %% 366 > 181 && step %% 366 < 213) {
          # July six-year average (2013-2018)
          Q_st = 6850/4
      }
      else if (step %% 366 > 212 && step %% 366 < 244){
          # Aug six-year average (2013-2018)
          Q_st = 8560/4
      }
      else if (step %% 366 > 243 && step %% 366 < 274) {
          # Sept six-year average (2013-2018)
          Q_st = 7580/4
      }
      else if (step %% 366 > 273 && step %% 366 < 305) {
          # Oct six-year average (2013-2018)
          Q_st = 6850/4 
      }
      else if (step %% 366 > 98 && step %% 366 > 129) { 
          # Nov six-year average (2013-2018)
          Q_st = 7830/4 
      }
      else if (step %% 366 > 128 && step %% 366 < 160) {
         # Dec six-year average (2013-2018)
         Q_st = 6120/4
      }
      else {
        # six-year monthly average (2013-2018)
         Q_st = 6500/4
      }
    }
    
    # the current year is defined as the rounded down integer year value of the current time step
    yr_curr = floor(t)
    
    # Use the number of years that have passed to determine the nitrogren loading based on the relative changes in the land use to the other land uses from the current condition. After 20 years, each of the scenarios will have reached their expected loading rate per that land use.
    if(t < 20){
      ...
      # weight of fish as a function of time based on von Bertalanffy growth parameters [g]
      W = Winf_mugil*(1-exp(-K_mugil*(t-t0_mugil)))^b_mugil 
    
      resp_o2 = 10^(-0.17*log10(W) + 2.49) # [mg O2 kg fish^-1 hr^-1] based on fish base equation for weight dependent relationship for respiration rate of M. cephalus
      ...
      #FB = W*N # [g fish/m^2] Biomass of fish in 
      
      
      ## PHYTOPLANKTON GROWTH - primarily derived from Burford and Lorenzen 2002
      # growth rate: g = g_alg_max * L_light * L_N 
      # phytoplankton growht is at its maximum except when limited by light or nitrogen
      g_alg_max = 1.45/4 # [6 hr^-1] max phytoplankton growth rate for 6 hour periods
      ...
      # These values are currently for non-leap years, 
      if (step %% 366 < 32) 
      {
        I0 = 144 # January
      }
      ...
      else if (step %% 366 > 304 && step %% 366 < 335){
        I0 = 145 # November
      }
      else if (step %% 366 > 334 && step %% 366 < 366){
        I0 = 135 # December
      }
      else {
        I0 = 186 # annual average solar irradiance near He‘eia Fishpond
      }
    
      I_sat = 60 # [W/m^2] saturating sunlight intensity converted from 275 μmol photons m−2 s−1 (Gao, et al, 2007) using this unit converter: http://www.egc.com/useful_info_lighting.php
      L_light = exp(1)/k_ext*(exp(-I0/I_sat*exp(-k_ext*z))) - exp(-(I0/I_sat))
      
      K_sN = 0.008 # [g m^-3] half saturation other k_SN is 0.015 g m^-3. The same variable defined in another source
      L_N = (Na_fp + Ni_fp)/(Na_fp + Ni_fp + k_SN)
  ...
  dNa_fp <- (Q_st*Na_st + Q_tide*(Na_ocean-Na_fp))/V_fp - k_oa*No_fp - (k_ao+k_nit+k_av+k_s)*Na_fp + k_r*Na_sd - g_alg/alg_CN*alg/h_fp*(Na_fp/(Na_fp+Ni_fp)) + (eg+exc)*fish/alg_CN/h_fp/alg_CN*(Na_fp/(Na_fp+Ni_fp))
  # Fishpond water nitrate [g/m^3]  
  dNi_fp <- (Q_st*Ni_st + Q_tide*(Ni_ocean-Ni_fp))/V_fp + k_nit*Na_fp-k_s*Ni_fp+k_r*Ni_sd - g_alg/alg_CN*alg/h_fp*(Ni_fp/(Na_fp+Ni_fp)) + (eg+exc)*fish/alg_CN/h_fp*(Ni_fp/(Ni_fp+Ni_fp))
  ...
  if (alg < 1)
    dalg <- g_alg*alg - k_s*alg - k_ra*alg
  else
    dalg <- g_alg*alg - k_s*alg - k_ra*alg - FA/alg_energy*fish 
  ...
  # Also account for fish density changes during spawning (lost mass for energy going to reproduciton but also gained mass from new fish)
  ...
  # there are reproducing fish after 2 years
  if (t > 2) {
    if(step %% 366 < 91 | step %% 366 < 334) { 
      ...
      catch_rate = k_catch # can harvest after 2 years during times of year outside of the closed season
    }
  }
  else {
  ...
  # dFE = (FA - FC)*W*N - (M+catch_rate)*FE # [kJ/day]  
  #dfish <- FA/alg_energy/alg*fish^2*(1-fish/Fcarr) - M*fish^2 - catch_rate*fish^2 
  

  #dfish <- FA/alg_energy/alg*fish^2*(1-fish/Fcarr) - M*fish^2 - catch_rate*fish^2 
  
  f_sgr = 1.02/4 # [6 hr^-1] Wassef, et al 2002 suggested ~2 % day^specific growth rate
  ...
  # The below equations are from: Lorenzen, K. 2008 "Fish Population Regulation Beyond "Stock and Recruitment": The Role of Density-dependent Growth in the Recruited Stock
...
# Nitrogen loading from current land use
...
## Fish energy consumption and excretion parameters
...
w_avg = 1000 # [g] average weight of M. cephalus
...
alg_CN = 6.6 # ration of C:N in algae based on the Refield ratio 106:16
...
# alternative energy content conversion for M. cephalus on Fishbase under the "Mass Conversion" tab
#  5.3 kJ/g WM; this would correspond to 


a_eg = 0.16 # 0.16 recommended by source. egestion percentage of consumption from Megrey, et al. 2007
a_ex = 0.1 # 0.1 recommended by source. excretion percentage of difference between consumption and egestion from Megrey, et al. 2007
eg = a_eg*FA/alg_energy # [g alg g fish^-1 6hr^-1] egestion rate
exc = a_ex*(FA/alg_energy - eg) # [g alg g fish^-1 6hr^-1] excretion rate


# Fish density initialization
...
Fcarr = 56 # [g/m^2] He‘eia Fishpond carrying capacity for fish
...
# here 2 years is used as the mature age to identify the mass of female fish at that age
egg_per_f <- egg_per_mass*Winf_mugil*(1-exp(-K_mugil*(2-t0_mugil)))^b_mugil
...
# juveniles per female surviving 1 year post hatching as they are recruited
recr_perf <- egg_per_f*0.22*M_mugil
...
pars_control <- c(
      ...
      max_mass_dens = 56, # [g/m^2] carrying capacity for fish in He‘eia Fishpond, 500 pounds per acre
      max_number_dens = 56/Winf_mugil, # [fish/m^2] carrying capacity in number of fish based on the maximum weight of M. cephalus based on von Bertalanffy growth
      ...
      k_catch = 1.2/365/4, # could be 0.343/yr based on Mexico study.
...
pars_urban <- c(
      ...
      # based on doubling time of 1.4-4.4 years, r_growth = ln(2)/1.4 = 0.495 /yr to ln(2)/4.4 = 0.158 /yr, prior r (whatever that means) is 0.51
      M = M_mugil/365/4, # [1/6hr] rate of fish death from empirical Then formula, M_mugil = 0.346
      k_catch = 1.2/365/4, # could be 0.343 based on Mexico study. file:///Users/Nakoa/Downloads/2016GallardoetalfisherydeMugelcephalusJMBO-5-151.pdf
      # fishbase also says you generally want Exploitation E to be about 0.5 which is F/Z = F/(F+M) so M and F should be approximately equal ideally for MSY
      # From Barman, Jana, Garg, Bhatnagar, https://link.springer.com/article/10.1007/s10499-004-2479-5
      # growth rate for M. cephalus in salinity 20 ppt 0.69 g/d or 0.67 g/d in 25 ppt
      # SGR --specific growth rate of weight-- (% BW (in grams) per day) about 4% per day
...
# ammonium (in terrogenous near egret island): 0.54 g/m^3 (~30 uM) - it's more like 25uM at mostin the first 1cm
...
state6 <- c(
           ...
           Na_sd = 0.45, # [g/m^3] fishpond sediment initial ammonium, sediment is a source for ammonium according to Hargreaves (1998)
           Ni_sd = 0.01, # [g/m^3]  fishpond sediment initial nitrate, sediment is a sink for nitrate according to Hargreaves (1998).
           alg = 0.018, # [g/m^2] initial algae concentration based on average of data from Frank, et al (0.0015 g Chl a/m^3) to g/m^2 algae. 0.0015 g Chla/m^3 * 30 carbon/Chl * 0.4
           # in 2007, Heeia FP algae level would also be about 0.018 g/m^2
           # 30 (w/w) carbon to chlorophyll ratio (Parsons et al 1984)
           # previously suggested algae to Chl ratio was 0.02 g algae/g Chla
           ...
           fish = 3, # [g/m^2] initial ‘ama‘ama  density
           catch = 0)
