...
# Every lunar cycle (approximately a month), there are two sets of spring and two sets of neap tides which totals to 128 tidal cycles. By using the modulo operator based on 128 tidal cycles, you can get all of the spring and neap cycles and all of the flood and ebb cycles in each month which will repeat each month.
    ...
    # the above loading doesn't seem to be an issue for the different scenarios but it's possible that the model is not sensitive to the current differences in nitrogen input
  ...
  dNa_fp <- (Q_st*Na_st + (k_oa*No_fp - (k_ao+k_nit+k_av+k_s)*Na_fp + k_r*Na_sd)*V_fp+Q_tide*
             (Na_ocean-Na_fp)-((r_NChl*k_ag*((Na_fp+Ni_fp)/(k_SN+No_fp+Ni_fp))+r_NChl*k_ra)*alg))/V_fp +              Na_exc*FB/h_fp
  ...
  dalg <- (k_ag*(Na_fp+Ni_fp)/(k_SN+Na_fp+Ni_fp) -k_ra)*alg - FA*FB/alg_energy # FB increases as W increases but N also decreases. If N was at 5000 and weight reached Winf_chanos, then the last term in this equation would be - 9.39 which is more than the initial alg concentration
  
  # Algae stocking rate in M. cephalus hatchery experimetn in Makapuu, HI would stock with 500,000 cells/mL of algae (phytoplankton Nannochloropsis oculata)

  # Fish density

  # define catch rate
  if(step < 365*2)
    catch_rate = 0 # No harvest for 2 years after stocking to allow the initial population to mature
  else
    catch_rate = k_catch # after the first 2 years, harvest at the defined first order rate
...
FA = 1446/1000/4 # [kJ g fish^-1 6h^-1] energy consumed per gram of M. cephalus every 6 hours from FAO website
...
pars_control <- c(
      ...
      k_catch = 604/365/4, # could be 0.343/yr based on Mexico study.
...
pars_restored <- c(
      ...
      #k_fg = K_m*ugil/365/4, # rate of fish growth [1/6hr], K_mugil = 0.27
      # based on doubling time of 1.4-4.4 years, r_growth = ln(2)/1.4 = 0.495 /yr to ln(2)/4.4 = 0.158 /yr, prior r (whatever that means) is 0.51
      M = M_mugil/365/4, # [1/6hr] rate of fish death from empirical Then formula, M_mugil = 0.346
      k_catch = 1.2/365/4, # could be 0.343/yr based on Mexico study. file:///Users/Nakoa/Downloads/2016GallardoetalfisherydeMugelcephalusJMBO-5-151.pdf
      # fishbase also says you generally want Exploitation E to be about 0.5 which is k_catch/Z = F/(F+M) so M and F should be approximately equal ideally for MSY
...
state6 <- c(
           ...
           alg = 0.3, # [g algae/m^2] initial algae concentration based on average of data from Frank, et al (0.0015 g Chl a/m^3) to g/m^2 algae. 0.0015 g Chla/m^3 / 0.02 g algae/g Chla* 0.4 m
