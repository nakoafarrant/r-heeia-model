...
fp_nitrate_y80 <- fp_water6 %>% filter(nitrogen == "Ni_fp") %>% 
  filter(time >= 80 & time < 81)


nitate_combo_y80 = ggplot(data = fp_nitrate_y80, aes(x = time, y = concentration)) +geom_line(col = 'green') + geom_point(data = nitrate_data, aes(x = day_ct/365+80, y = nit_mean),colour='blue')
...
fp_amm_y80 <- fp_water6 %>% filter(nitrogen == "Na_fp") %>% 
  filter(time >= 80 & time < 81)


amm_combo_y80 = ggplot(data = fp_amm_y80, aes(x = time, y = concentration)) +geom_line(col = 'red') + geom_point(data = ammonium_data, aes(x = day_ct/365+80, y = amm_mean),colour='magenta')
