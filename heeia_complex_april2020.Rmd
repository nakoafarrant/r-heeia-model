...
# We are initializing this model to conditions on Jan. 1, 2014. The moon phase was a new moon representing the start of a spring tide cycle. The tide was high (flood conditions) in the early morning hours of 1/1/2014, so we begin with a spring flood tide and iterate through the cycle from there.
    ...
    # Q_st is defined below in units of m^3 / 6 hours from m^3 / day
      ...
      c_k4 = 2.5e-4 # (mol N/m^3) half-saturation constant for the uptake of ammonium and nitrate by pelagic autotrophs (2.5e-4 comes from Humborg et al 2000)
      c_k7 = 2.5e-4 # (mol N/m^3) half-saturation constant for the uptake of ammonium and nitrate by benthic autgtrophs
      ...
      d6 = (2.3e-4)/365/4 # (1/6hr) mortality rate of pelagic heterotrophs
      ...
      P4 = 6.4*10^4*0.0018 # (mol N), half-saturation constant of nutrient assimilation kinetics by pelagic autotrophs; sum of the ammonium and nitrate reservoirs
      
      #P5 = 7.67e5*0.0018
      #P5 = 600*0.0018 #this made the peak value for N5_fp larger than when P5 uses 5.4e5, but the shape of the graph stayed about the same with N5 crashing by 50 years
      # original P5 below
      P5 = 5.4*10^5*0.0018 # (mol N) all of these half saturation constants were calculated relative to the size of the reservoirs in the Southern Kaneohe Basin which has more than 100x the volume of Heeia Fishpond. The 0.0018 is a correction factor relative to the volume ratio of Heeia FP: Kaneohe Bay southern basin assuming that the two water bodies have similar compositions
      P6 = 2.0*10^5*0.0018 # (mol N)
      P7 = 6.4*10^4*0.0018 # (mol N)
      ...
      K6 = ((N3_fp + N4_fp + N5_fp)*V_fp)^c_MM/(((N3_fp + N4_fp + N5_fp)*V_fp)^c_MM + P6^c_MM)
      ...
      dN6_fp <- (NF_36 + NF_46 + NF_56 - NF_61a)/V_fp - NF_63 - catch_rate*N6_fp
...
Stream: org N stabilizes within 30 years to around 1.5 with little fluctuation. Ammonium and nitrate increase generally for 10-20 years then reach a stable point around 0.11 and 0.08, respectively. These stream values are considerably higher than the initial values and could be causing other issues in other model compartments

Fishpond Water: 
Organic N stays low near 0.5 or 1 for about 40 years and then starts increasing exponentially from year 50 to about 80 where it reaches a value of about 8 and then starts gradually decreasing. Still on the decline from year 100 to 200

Ammonium stays low near 0.02 for 40 years then has a massive spike to about 2.5 around year 50 before dropping right back off again to 0.02. Similarly see nitrate at about 0.07 and then rapidly increases to about 60 starting around year 40 and then drops off again to the very low level by about year 60. These spikes aligns pretty closely with the spike observed in stream ammonium and nitrate.

Sediment:
organic N looks very similar in shape to the fishpond water org. N except the concentrations about about 10 times higher in the sediment. Similarly see much higher 
...
```
Before the control situation fails, nitrate in the fishpond is increasing at a steady clip and algae (N4) seems like it has been pretty stable
